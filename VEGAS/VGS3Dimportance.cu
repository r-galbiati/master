...
#define CEXP 0.1                  // Parameter of our exponential (trial) function
...
struct function {
  // Method to evaluate the function in a single point
  // It is both __host__ and __device__ because it will get called both on the CPU and GPU
  __host__ __device__ float getValue (float x,float y,float z) { return (  pow((1./(CEXP*sqrt(PI))),D)*exp(-(pow((x-0.5),2)+pow((y-0.5),2)+pow((z-0.5),2))/(CEXP*CEXP)));}
};
...
__global__ void WeightsX (curandState_t* states, function *tempFunction, pdfStep *temp, float *tempColumn) {
  ...
  cache[cacheIndex] = fabs(tempFunction->getValue(xvalue,curand_uniform(&states[tid]),curand_uniform(&states[tid])));
...
void AdjustX (pdfStep *temp, float *tempColumn, float &tempColumnTotal, float &tempDeltaTot) {
  // DeltaTot needs to be equal to 0
  tempDeltaTot=0;
...
__global__ void WeightsY (curandState_t* states, function *tempFunction, pdfStep *temp, float *tempColumn) {
  ...
  cache[cacheIndex] = fabs(tempFunction->getValue(curand_uniform(&states[tid]),yvalue,curand_uniform(&states[tid])));
...
void AdjustY (pdfStep *temp, float *tempColumn, float &tempColumnTotal, float &tempDeltaTot) {
...
__global__ void WeightsZ (curandState_t* states, function *tempFunction, pdfStep *temp, float *tempColumn) {
  ...
  cache[cacheIndex] = fabs(tempFunction->getValue(curand_uniform(&states[tid]),curand_uniform(&states[tid]),zvalue));
...
// This works on the CPU
void AdjustZ (pdfStep *temp, float *tempColumn, float &tempColumnTotal, float &tempDeltaTot) {
...
//---------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    WeightsX<<<N,threadsPerBlock>>>(states, dev_example, dev_pdf, dev_column);
    ...
    AdjustX (pdf, column, columnTotal, deltaTot);
...
//---------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    WeightsY<<<N,threadsPerBlock>>>(states, dev_example, dev_pdf, dev_column);
    ...
    AdjustY (pdf, column, columnTotal, deltaTot);
...
//---------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    WeightsZ<<<N,threadsPerBlock>>>(states, dev_example, dev_pdf, dev_column);
    ...
    AdjustZ (pdf, column, columnTotal, deltaTot);
